...
import { KaagazUser, APIURLS, Response, KaagazUserRes } from 'kaagaz-models';
...
export class LoginService {
     ...
     * @param otp required
     */
    verifyOTP(phoneNumber: string, otp: string): Observable<KaagazUser> {
        const body = { otp: otp, phoneNumber: phoneNumber }
        const url = APIURLS.VERIFY_OTP;
        return this._http.postResponse<KaagazUserRes>(KaagazUserRes, url, body).pipe(
            tap((res: KaagazUserRes) => {
            ...
            map((res: KaagazUserRes) => { return !res.error && res.data ? res.data : null; })
        );
    }
}